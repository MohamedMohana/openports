...
public class PortHistoryTracker {
    ...
    func isCommonPort(_ port: Int, threshold: Int = 5) -> Bool {
        ...
        let portOccurrences = history.flatMap(\.self).count(where: { $0.port == port })
    ...
    func getAverageUptime(for port: Int) -> TimeInterval? {
        ...
        let history = loadHistory()
        let portEntries = history.flatMap(\.self).filter { $0.port == port }
    ...
    private func loadHistory() -> [[PortHistoryEntry]] {
        guard let data = userDefaults.data(forKey: historyKey),
              let history = try? JSONDecoder().decode([[PortHistoryEntry]].self, from: data)
        else {
            return []
        }
        return history
